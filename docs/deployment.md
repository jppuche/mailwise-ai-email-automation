...
| `JWT_ACCESS_TTL_MINUTES` | No | `15` | Access token lifetime (minutes). production-critical default |
| `JWT_REFRESH_TTL_DAYS` | No | `7` | Refresh token lifetime (days) |
| `BCRYPT_ROUNDS` | No | `12` | bcrypt work factor. production-critical default. Too low: brute-force risk. Too high: login >1s |
...
| `LLM_TEMPERATURE_CLASSIFY` | No | `0.1` | Keep low — high temp causes inconsistent classifications |
| `LLM_TEMPERATURE_DRAFT` | No | `0.7` | Keep moderate — low temp produces robotic drafts |
...
| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `POLLING_INTERVAL_SECONDS` | No | `300` | Gmail polling frequency. Too low = Gmail rate limit |
| `INGESTION_BATCH_SIZE` | No | `50` | Emails per ingestion run. Too high = LLM timeout |
| `MAX_BODY_LENGTH` | No | `4000` | Email body truncation before LLM. Affects token usage and context |
