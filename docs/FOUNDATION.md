# Intelligent Email Classification and Routing System — Product Requirements Document (PRD)

**Purpose**: Product requirements document describing WHAT the system does, WHY, and under what constraints. Technology-agnostic by design — this document guided implementation without prescribing specific frameworks or libraries.

**Format**: Markdown — optimized for readability (renders in GitHub, VS Code, any modern editor) with structured headers, tables, code blocks, and hierarchical organization.

**Version**: 1.0 | **Date**: 2026-02-19

> This document is the single source of truth for product requirements. Section 13 defines feature priority tiers. Section 14 documents lessons from a predecessor project. Appendix B defines data contracts. Appendix D maps this specification to the final implementation.
...
14. [Lessons from Predecessor Project](#14-lessons-from-predecessor-project)
...
- **Real business problem** — Solve email triage for mid-size organizations, not a toy demo
- **Production-ready architecture** — Retry logic, graceful degradation, structured logging, health checks
- **AI with guardrails** — LLM classification with heuristic fallback, prompt injection defense, human-in-the-loop review
- **Extensible by design** — Adapter pattern across all integrations: swap providers without touching business logic
- **Observable and debuggable** — Every email traceable through the pipeline via correlation IDs and explicit state machine
...
The deploying organization can:
...
### 7.1 HITL Requirement — Non-Negotiable
...
|---|---|
| Email ingestion from Gmail | Core functionality. At least one provider adapter. |
| Two-layer classification with LLM | Core differentiator. Demonstrates LLM integration. |
| Configurable categories via dashboard | Real business design, not hardcoded. |
| Input sanitization + prompt injection defense | Security-first design for LLM systems. |
| Fallback classification + confidence flagging | Production-readiness. |
| Routing rules engine (configurable) | Core business logic. |
| At least one channel adapter (Slack) | End-to-end routing pipeline. |
| Response draft generation with HITL review | AI safety + responsible AI design. |
| Web dashboard: email browser, review queue, config | Full-stack capability. |
| REST API for all operations | API design. |
| Structured error handling with retry logic | Production-readiness. |
| Docker-based deployment | Cloud-readiness. |
| Env-based configuration (no hardcoded creds) | Security baseline. |
| Deduplication / idempotent processing | Reliability baseline. |
| Basic analytics (classification distribution, volume) | Observability. |

### Tier 2 — RECOMMENDED (Differentiation)

| Feature | Justification |
|---|---|
| CRM integration (at least one adapter) | Enterprise integration. |
| Heuristic pre-classification rules | Defense-in-depth for accuracy. |
| Rule testing mode (simulate for sample email) | UX sophistication. |
| Analytics with time-series charts | Data visualization. |
| Confidence scoring with review queue | Nuanced AI-system design. |
| Health check endpoint + structured logging | Ops/SRE awareness. |
| Unit + integration test suite | Testing discipline. |
| Adapter architecture docs + guide for new adapters | Architectural thinking. |
| Email thread awareness | Email domain depth. |
| VIP sender priority escalation | Real business need. |
| CSV export for analytics | Low-cost high-value feature. |
| Classification feedback loop (few-shot) | MLOps thinking: human overrides become few-shot examples in the classification prompt. No model fine-tuning — prompt-level learning only. |
| Dashboard dark mode | Theme system with CSS tokens, persistent preference, toggle component. |

### Tier 3 — OPTIONAL (Polish)

| Feature | Justification |
|---|---|
| Multiple email accounts simultaneously | Realistic but complex. |
| Bulk reclassification from dashboard | Power-user feature. |
| Webhook-based push (vs polling) | Advanced integration pattern. Requires public endpoint and subscription management. |
| Multi-language classification | Internationalization. |
| Admin action audit log | Compliance. |

---

## 14. Lessons from Predecessor Project

> This system evolved from an earlier n8n-based email organizer that validated core patterns against 80+ real emails. The lessons below shaped key architectural decisions.
...
7. **Independent review catches what the implementer misses**: A structured security audit (11 dimensions) caught 7 warnings + 8 suggestions the implementer missed. → Code review against specs catches mismatches testing alone doesn't reveal.
...
All configuration is environment-variable based via Pydantic `BaseSettings`. See [Deployment Guide](deployment.md) for the complete variable reference (60+ variables with defaults and descriptions).

### Appendix D: Implementation Status

This specification was fully implemented. Summary:

| Spec Section | Status | Implementation Detail |
|---|---|---|
| Email Ingestion (Sec 2.1) | Complete | Gmail adapter, OAuth2 with token refresh, incremental sync, dedup by message ID |
| Two-Layer Classification (Sec 4) | Complete | 4 actions, 10 types, LLM via LiteLLM + heuristic fallback, few-shot learning from human feedback |
| Routing Engine (Sec 5) | Complete | 6 condition operators, multi-rule evaluation, priority override, Slack Block Kit notifications |
| CRM Integration (Sec 6) | Complete | HubSpot adapter, idempotent 6-operation chain (lookup, create contact, log activity, create deal, update fields, test connection) |
| Response Drafts (Sec 7) | Complete | LLM drafts with org context + CRM context, HITL review queue, never auto-send |
| Dashboard (Sec 8) | Complete | 12 pages, React 19 + TypeScript, Tailwind CSS v4 + shadcn/ui, dark mode, real-time analytics |
| Adapter Architecture (Sec 9) | Complete | 4 adapter families (Email, Channel, CRM, LLM), all behind abstract base classes with typed schemas |
| Prompt Injection Defense (Sec 11.2) | Complete | All 5 layers: input sanitization, defensive prompts, data delimiters, output enum validation, no tool access |
| State Machine (Sec 3.4) | Complete | 12 DB-enforced states, explicit transitions, named error states per pipeline stage |
| Edge Cases (Sec 10) | Complete | 30+ scenarios handled per specification |

**Testing:** 1,780+ backend tests, 342 frontend tests, 93% code coverage, E2E pipeline validation.
**Infrastructure:** 6 Docker services with health checks, CI/CD via GitHub Actions, structured JSON logging.
**Tier coverage:** All Tier 1 (MVP) and Tier 2 (Differentiation) features implemented. Tier 3 partially scoped.
