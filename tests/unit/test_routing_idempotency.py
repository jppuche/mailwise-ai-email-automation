...
try-except D7: DB is an external-state boundary — AsyncMock simulates it.
pre-mortem Cat 6: each RoutingAction commits independently — tested via commit call counts.
"""
...
class TestDispatchIdempotency:
    ...
    async def test_skip_already_dispatched_action(self) -> None:
        ...
        Spec B09: idempotency check via DB, never CRM/channel API.
        """
    ...
    async def test_idempotency_check_db_error_counts_as_failed(self) -> None:
        ...
        try-except D7: external-state failures captured, not propagated.
        """
        db = AsyncMock()
