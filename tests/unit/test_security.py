...
Architecture directives tested:
- tighten-types D1: TokenPayload is TypedDict with exactly sub/role/exp keys.
- try-except D7: verify_access_token has 3 separate except blocks for
  ExpiredSignatureError, JWTClaimsError, JWTError.
- try-except D8: verify_password uses conditional return, NOT try/except.
"""
...
class TestVerifyPassword:
    ...
    try-except D8: this is local computation — must use conditional return,
    not try/except.
    """
    ...
    def test_uses_conditional_not_try_except(self) -> None:
        ...
        assert "try" not in source, (
            "verify_password must not use try/except (try-except D8: local computation)"
        )
...
class TestVerifyAccessToken:
    ...
    try-except D7: 3 separate except blocks for ExpiredSignatureError,
    JWTClaimsError, JWTError. Missing-claims check uses a conditional.
    """
...
class TestTokenPayload:
    ...
    tighten-types D1: no dict[str, Any] at this boundary.
    """
