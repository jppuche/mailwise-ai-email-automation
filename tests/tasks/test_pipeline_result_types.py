...
Validates that all 5 frozen dataclasses satisfy tighten-types D1/D3:
  - D1: No ``Any`` fields -- all annotations are fully typed.
  - D3: Results stored in DB/typed dataclasses, NOT via Celery result backend
...
# ---------------------------------------------------------------------------
# D3 compliance: AsyncResult must NOT appear in src/tasks/
# ---------------------------------------------------------------------------


class TestAsyncResultAbsence:
    """D3: Task results stored in DB/typed dataclasses — not via AsyncResult.get().
    ...
    def test_async_result_absent_from_tasks_package(self) -> None:
        ...
        assert violations == [], (
            ...
            "dataclasses stored in DB (D3). Violations:\n" + "\n".join(violations)
        )
