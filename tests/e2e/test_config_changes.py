...
def test_new_category_available_for_classification(migrated_db: None) -> None:
    ...
    Verifies DB FK integrity for dynamic categories.
    """
...
def test_new_routing_rule_activates_for_next_routing(migrated_db: None) -> None:
    ...
    Verifies that rules added between pipeline runs take effect immediately on
    the next run (no caching).
    """
    session_factory = _make_session_factory()
...
def test_disabled_routing_rule_skipped(migrated_db: None) -> None:
    ...
    Verifies that the is_active flag is respected at runtime, not just at
    rule-creation time.
    """
    session_factory = _make_session_factory()
