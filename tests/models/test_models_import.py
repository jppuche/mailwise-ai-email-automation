...
tighten-types (D1): TypedDicts are verified importable — they document the
JSONB field contracts. If a TypedDict is removed or renamed, these tests fail.
"""
...
class TestTypedDictsImportable:
    ...
    tighten-types D1: TypedDicts are the JSONB field contracts. They ensure
    that write paths to JSONB columns are type-checked by mypy. If a TypedDict
    is missing, JSONB fields degrade to untyped dict[str, Any].
    """
...
class TestMappedAnnotations:
    ...
    tighten-types D1: No model column should use legacy Column() without Mapped[].
