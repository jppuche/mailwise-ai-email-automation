...
pre-mortem Cat 3 (stringly-typed): Categories in DB are canonical. A typo in
a slug here catches a migration error early, before the classification service
tries to resolve slugs to IDs and silently falls through to the fallback.
...
class TestActionCategorySeedData:
    ...
    def test_action_urgent_exists(self, db_session: Session) -> None:
        """'urgent' category must be present — required by B08 classification."""
        from src.models.category import ActionCategory
