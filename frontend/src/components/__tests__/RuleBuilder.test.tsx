...
describe("RuleBuilder — create mode (no rule prop)", () => {
  ...
  it("renders the 'New Routing Rule' title in create mode", () => {
    ...
    expect(screen.getByRole("dialog", { name: /create routing rule/i })).toBeInTheDocument();
...
describe("RuleBuilder — active toggle", () => {
  ...
  it("active checkbox is checked by default in create mode", () => {
    const onSubmit = vi.fn();
    const onClose = vi.fn();
    render(<RuleBuilder onSubmit={onSubmit} onClose={onClose} />);

    // All checkboxes in the form — the active toggle is the only checkbox
    // (condition rows use select+input, not checkboxes)
    const checkboxes = screen.getAllByRole("checkbox");
    // The active toggle checkbox has no aria-label; condition/action rows use selects
    // There is exactly 1 checkbox rendered (the is_active toggle)
