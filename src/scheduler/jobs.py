...
``IngestionService._acquire_poll_lock`` (B07).
...
def _get_active_accounts() -> list[str]:
    ...
    records. For now (B12), returns a configurable list. The scheduler
    does NOT import ``src.api`` — it uses its own DB access if needed.
    """
    # TODO(B13+): Query EmailAccount table for active accounts
    # For now, return empty — scheduler is wired but accounts come from config
    return []
