...
try-except D7: Every SDK call is wrapped in ``asyncio.to_thread()`` and
  uses structured try/except for ``ApiException``.
try-except D8: Argument validation and SDK-to-Pydantic mapping use
  conditionals, not try/except.
"""
...
class HubSpotAdapter(CRMAdapter):
    ...
    async def connect(self, credentials: CRMCredentials) -> ConnectionStatus:
        ...
        # D7: external-state operation — verify token with a lightweight call
        try:
    ...
    async def lookup_contact(self, email: str) -> Contact | None:
        ...
        # Local computation — conditionals (D8)
    ...
    async def log_activity(self, contact_id: str, activity: ActivityData) -> ActivityId:
        ...
        # Build note properties (local computation — no try/except)
        note_body = (
        ...
        # D7: external-state operation — create note
        try:
        ...
        # D7: external-state operation — associate note to contact
        try:
            await asyncio.to_thread(
    ...
    async def create_lead(self, data: CreateLeadData) -> LeadId:
        ...
        # D7: external-state operation — create deal
        try:
            response = await asyncio.to_thread(
