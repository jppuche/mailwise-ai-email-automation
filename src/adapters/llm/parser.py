...
try-except D8: Pure local computation. All logic uses conditionals except
``_safe_json_loads`` (json.loads has no conditional alternative).

Handles 7 documented output shapes (Cat 4 pre-mortem):
  1. Pure JSON
...
def _safe_json_loads(text: str) -> dict[str, object] | None:
    ...
    Note: json.loads has no conditional alternative — this is the single
    exception to the D8 no-try/except rule for this module.
    """
    try:
