...
Top-level Celery handler is the ONLY place bare ``except Exception`` is
permitted, to ensure task retry on unexpected failures.
"""
...
def ingest_emails_task(account_id: str, since_iso: str) -> None:
    ...
    Note: This is a plain function, not a Celery task decorator. The Celery
    pipeline registers it with the app and adds retry logic.
    """
...
async def _run_ingestion(
    ...
    This is intentionally minimal — imports are deferred to avoid
    ...
    try:
        async with AsyncSessionLocal() as session:
            # Placeholder: adapter instantiation will come from account configuration
