...
updates → persist CRMSyncRecord with independent commit.
...
Guarantees: sync() always returns a CRMSyncResult. CRMSyncRecord is
  committed independently — persists even if caller fails after.
...
CRMSyncRecord committed independently — partial failure recorded.
All defaults from CRMSyncConfig (sourced from Settings env vars).
"""
...
def _compute_overall_success(operations: list[CRMOperationStatus]) -> bool:
    ...
    Pure local computation — conditionals only.
...
class CRMSyncService:
    ...
    Invariants:
      ...
      - ``config`` provides all configurable defaults.

    Guarantees:
      - ``sync()`` always returns a ``CRMSyncResult``.
      - ``CRMSyncRecord`` is committed independently.
    ...
    async def sync(
        ...
        Guarantees:
          ...
          - ``CRMSyncRecord`` is committed to DB before returning.
        ...
        # Step 3: Contact create (conditional)
        ...
        # Step 7: Persist sync record (independent commit)
    ...
    async def _check_idempotency(
        ...
        SQLAlchemyError caught and returns None, treating as fresh sync.
    ...
    async def _do_contact_lookup(
        ...
        CRMAuthError and CRMRateLimitError are re-raised.
    ...
    async def _do_activity_log(
        ...
        Snippet is truncated to activity_snippet_length before sending.
    ...
    async def _do_field_updates(
        ...
        Per-field try/except: each field update is independent.
        ...
        Note: update_field() returns None — bare await.
    ...
    async def _persist_sync_record(
        ...
        *,
        ...
        """Persist a CRMSyncRecord with its own independent commit.

        SQLAlchemyError triggers rollback + re-raise.
