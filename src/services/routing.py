...
contract-docstrings:
  ...
  Guarantees: route() transitions email to ROUTED or ROUTING_FAILED. Each RoutingAction
    has its own db.commit() (D13). Partial failure in action N does not revert N-1.
...
pre-mortem:
  Cat 6: Each RoutingAction commits independently — failure in N does not revert N-1.
  Cat 1: Single state transition at end (ROUTED or ROUTING_FAILED), not per-action.
  Cat 8: All defaults from Settings (env vars), no hardcoded values in service.
"""
...
# VIP escalation keywords (Cat 3: module-level frozenset, not free-form strings)
...
class RoutingService:
    ...
    Invariants:
      ...
      - ``settings`` provides all Cat 8 configurable defaults.

    Guarantees:
      - ``route()`` transitions email to ROUTED or ROUTING_FAILED.
      - Each RoutingAction has its own ``db.commit()`` (D13).
    ...
    async def route(
        ...
        # Determine final state — local computation (D8)
        if len(matches) == 0:
        ...
        # Transition email state — external state (D7)
    ...
    async def _build_routing_context(
        ...
        External state (D7): reads ClassificationResult, ActionCategory, TypeCategory.
        """
    ...
    async def _dispatch_rule_actions(
        ...
        for action_def in match.actions:
            ...
            # Check idempotency — external state (D7)
            try:
            ...
            # Resolve adapter — local computation (D8)
            ...
            # Build payload — local computation (D8)
            payload = self._build_routing_payload(context, action_def, match)

            # Dispatch — external state (D7)
            ...
            # Record success — external state (D7)
    ...
    async def _record_failed_action(
        ...
        *,
        ...
        """Record a failed RoutingAction with its own commit (D13)."""
        priority = _determine_dispatch_priority(context, match.priority, self._vip_senders)
    ...
    async def _record_success_action(
        ...
        *,
        ...
        """Record a successful RoutingAction with its own commit (D13)."""
        priority = _determine_dispatch_priority(context, match.priority, self._vip_senders)
