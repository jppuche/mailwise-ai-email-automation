...
All operations are local computation: string assembly, conditional checks,
data extraction. No try/except blocks.
...
class DraftContextBuilder:
    ...
    def build(
        ...
        Local computation only — zero try/except.
        """
        notes: list[str] = []
    ...
    def _extract_crm_context(self, record: CRMSyncRecord) -> CRMContextData:
        ...
        Only ``contact_id`` is available from the record.
    ...
    def build_llm_prompt(self, context: DraftContext) -> str:
        ...
        Local computation — string assembly only.
        """
        sections: list[str] = []
