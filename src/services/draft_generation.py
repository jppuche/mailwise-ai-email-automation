...
Guarantees: generate() always returns a DraftResult. Draft is committed before
...
Draft committed independently — Gmail push failure does not lose draft.
All defaults from DraftGenerationConfig (sourced from Settings env vars).
"""
...
class DraftGenerationService:
    ...
    Guarantees:
      ...
      - Draft is committed before Gmail push attempt.
    ...
    async def generate(
        ...
        Guarantees:
          ...
          - Draft persisted in DB before any Gmail push.
        ...
        # Step 1: Load CRM record from DB (optional, for context)
        crm_record = await self._load_crm_record(email_id, db)

        # Step 2: Load template from DB if template_id set (not yet implemented)
        template_content: str | None = None

        # Step 3: Build context — delegated to DraftContextBuilder
        ...
        # Step 6: Create Draft ORM and persist
        draft_id = uuid.uuid4()
        draft = Draft(
            id=draft_id,
            email_id=email_id,
        ...
        try:
            db.add(draft)
            await db.flush()
            # Step 7: COMMIT — draft persists before push
            await db.commit()
    ...
    async def _load_crm_record(
        ...
        SQLAlchemyError caught, returns None.
        """
        try:
    ...
    async def _handle_failure(
        ...
        *,
        ...
        SQLAlchemyError on state transition is logged, not re-raised.
        """
        try:
    ...
    async def _push_to_gmail(
        ...
        EmailAdapterError caught, logged, returns failure tuple.
        Gmail push failure → DRAFT_GENERATED (NOT DRAFT_FAILED).
