...
class ClassificationService:
    ...
    async def classify_email(
        ...
        Preconditions:
          ...
          - email.body_plain is not None (B07 guarantees this for SANITIZED).
        ...
        # Build classification request — local computation (D8)
        ...
        # Run heuristics — local computation (D8)
        ...
        # Call LLM adapter — external state (D7)
        ...
        # Validate slugs against DB categories — local computation (D8)
        ...
        # Determine final confidence — local computation (D8)
        ...
        # Persist result and transition — external state (D7)
    ...
    async def _call_llm_or_fail(
        ...
        try:
            ...
            # Transition to error state — external state (D7)
            try:
    ...
    async def _persist_and_transition(
        ...
        External state (D7) — raises SQLAlchemyError on failure.
        """
        # Resolve slugs to category IDs — local computation (D8)
...
def _has_heuristic_disagreement(
    ...
    Local computation (D8) — conditionals, no try/except.
    """
