...
Guarantees: Per-email isolation — one failure does not affect others.
  Two independent commits per email (FETCHED then SANITIZED).
...
class IngestionService:
    ...
    async def _process_single_email(
        ...
        # 3d. Map adapter schema → ORM model
        ...
        # 3d. Store in DB → state=FETCHED → commit
        try:
        ...
        # 3e. Transition to SANITIZED → commit
        # transition_to() is local — InvalidStateTransitionError is a bug, NOT caught
